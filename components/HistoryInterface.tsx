    ...
    rounds.forEach(r => {
      if (r.status === 'COMPLETED' && r.scorecard) {
        ...
        if (activeTab === 'AI') {
          ...
          // In PvP, check if winnerName matches user or handle Draws
          // Note: Since we don't have username here, we rely on basic logic or 'Draw'
          if (sc.winner === 'Draw') draws++;
          else if (sc.winnerName && sc.winnerName !== 'AI') wins++; 
          // (Simplified win logic for display purposes)
        }
      }
    });
